...
KNOWN_BRANDS = ["Samsung", "Apple", "Xiaomi", "Honor", "Motorola", "Huawei", "Jota"]
...
def get_or_create_store(base_url, service_key):
    ...
    created = supabase_request(
        ...
        payload={"name": STORE_NAME, "website_url": "https://www.centralshop.com.py"},
    )
    return created[0]["id"]
...
def get_or_create_product(base_url, service_key, title, image_url):
    escaped = title.replace(" ", "%20").replace('"', "%22").replace("/", "%2F")
    existing = supabase_request(
